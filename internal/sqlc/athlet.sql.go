...
const getAllAthletenForVerein = `-- name: GetAllAthletenForVerein :many
SELECT DISTINCT
...
func (q *Queries) GetAllAthletenForVerein(ctx context.Context, vereinUuid uuid.UUID) ([]Athlet, error) {
	rows, err := q.db.Query(ctx, getAllAthletenForVerein, vereinUuid)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const getAthlet = `-- name: GetAthlet :many
SELECT
  athlet.uuid, athlet.vorname, athlet.name, athlet.geschlecht, athlet.jahrgang, athlet.gewicht, athlet.startberechtigt, athlet.verein_uuid,
...
func (q *Queries) GetAthlet(ctx context.Context, argUuid uuid.UUID) ([]GetAthletRow, error) {
	rows, err := q.db.Query(ctx, getAthlet, argUuid)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
